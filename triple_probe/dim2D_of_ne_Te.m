...
% shotlist = [46, 49, 52, 58, 61, 66, 69, 72]; % case-O
% shotlist = [72];
% スプレッドシートからR方向データを取得
    ...
    if exist(filename, 'file')
       ...
       % Te 計算
        try
            ...
            % ここで Te_values_raw_from_solve の状態を再確認するデバッグコード (前回の回答参照)
            % 例: disp(['DEBUG: Min Te_raw = ', num2str(min(Te_values_raw_from_solve(:))), ' Max Te_raw = ', num2str(max(Te_values_raw_from_solve(:)))]);
            % 例: disp(['DEBUG: Number of NaNs in Te_raw: ', num2str(sum(isnan(Te_values_raw_from_solve(:))))]);

            Te_values = Te_values_raw_from_solve;
            max_physical_Te = 15 * K2ev; % 15 K 以上は異常値とみなす。
            Te_values(Te_values < 0 | Te_values > max_physical_Te) = NaN; 
        
        catch ME 
            % ここにまだ入る場合、Te_symbolicの解析がまだ不十分か、別の数値安定性の問題がある
        ...
        if shot == shotlist(4) % あるいは確認したい特定のショット番号を指定 (例: if shot == 46)
            ...
            % あるいは (2 * I2_values + I3_values) ./ (I2_values + 2 * I3_values);
...
for i_shot = 1:size(Te_matrix, 1)
    % NaN値の補間 (時間方向)
    % 'linear' または 'spline' を試す
    Te_matrix(i_shot, :) = fillmissing(Te_matrix(i_shot, :), 'linear'); 
    ne_matrix(i_shot, :) = fillmissing(ne_matrix(i_shot, :), 'linear');
    
    % ここで時間方向のスムージングを追加することも検討（オプション）
    % Te_matrix(i_shot, :) = smoothdata(Te_matrix(i_shot, :), 'movmedian', 5); 
    % ne_matrix(i_shot, :) = smoothdata(ne_matrix(i_shot, :), 'movmedian', 5);
end
...
% NaN値を補完 (線形補完)
% Te_matrix = fillmissing(Te_matrix, 'linear', 2); % 時間方向で補完
% ne_matrix = fillmissing(ne_matrix, 'linear', 2); % 時間方向で補完
