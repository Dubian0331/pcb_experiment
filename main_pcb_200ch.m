...
addpath("C:\Users\w-har\OneDrive - The University of Tokyo\Lab\pcb_experiment")
setenv("pcb_raw_path","C:\Users\w-har\OneDrive - The University of Tokyo\Lab\pcb_experiment\raw_data");
setenv('pre_processed_directory_path', ...
  'C:\Users\w-har\OneDrive - The University of Tokyo\Lab\pcb_experiment\processed_data');

pathname.ts3u=getenv('ts3u_path');%old-koalaのts-3uまでのパス（mrdなど）
pathname.fourier=getenv('fourier_path');%fourierのmd0（データックのショットが入ってる）までのpath
pathname.NIFS=getenv('NIFS_path');%resultsまでのpath（ドップラー、SXR）
pathname.save=getenv('s avedata_path');%outputデータ保存先
pathname.rawdata38=getenv('rawdata038_path');%dtacq a038のrawdataの保管場所
pathname.woTFdata=getenv('woTFdata_path');%rawdata（TFoffset引いた）の保管場所
pathname.rawdata=getenv('pcb_raw_path');%dtacqのrawdataの保管場所
