...
% ★★★ 追加: ステップ1で保存した圧力勾配データをロード ★★★
load('C:\Users\w-har\OneDrive - The University of Tokyo\Lab\pcb_experiment\processed_data\grad_P_data_240610.mat');

% target_times = [91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103];
% target_times = [93, 107, 121];
target_times = [81, 86, 91, 96];
...
for t_idx = 1:length(target_times)
    ...
    % 径方向成分
    % プロット (z = 0) caseI
    plot(r(:,1), j_B_r(:, 10), 'DisplayName', ['t=', num2str(target_time+399), 'us'], 'LineWidth', 3); % 凡例用ラベル

    % ★★★ 追加: 対応する時刻の圧力勾配をプロット ★★★
    % 1. JxBの時刻 (us) を計算
    time_in_us = target_time + 399;
    
    % 2. grad_Pのデータから最も近い時刻のインデックスを探す
    [~, grad_P_time_idx] = min(abs(time_axis - time_in_us));
    
    % 3. その時刻の圧力勾配データを取得
    grad_P_to_plot = grad_P_2D(grad_P_time_idx, :);
    
    % 4. grad_PのR軸(R_axis)をJxBのR軸(r_jxb)に補間して合わせる
    %    圧力勾配は力なので、-1をかけてプロットするのが一般的
    grad_P_aligned = interp1(R_axis, -grad_P_to_plot, r, 'pchip', 'extrap');
    
    % 5. 補間した圧力勾配をプロット
    plot(r, grad_P_aligned, 'DisplayName', ['-grad P at t=', num2str(time_in_us), 'us'], 'LineStyle', '--', 'LineWidth', 2);



    % % プロット (z = 0) caseO
    % plot(r(:,1), j_B_r(:, 12), 'DisplayName', ['t=', num2str(target_time+399), 'us'], 'LineWidth', 3); % 凡例用ラベル

    j_B_t = j_B_force(:, :, 2); % toroidal成分
    % トロイダル方向成分
    % caseI
    % plot(r(:,1), j_B_t(:, 10), 'DisplayName', ['t=', num2str(target_time+399), 'us'], 'LineWidth', 3); % 凡例用ラベル
    % プロット (z = 0) caseO
    % plot(r(:,1), j_B_t(:, 10), 'DisplayName', ['t=', num2str(target_time+399), 'us'], 'LineWidth', 3); % 凡例用ラベル
